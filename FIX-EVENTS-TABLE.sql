-- ===================================
-- FIX EVENTS TABLE - Change 'date' to 'event_date'
-- ===================================
-- Run this in Supabase SQL Editor

-- 1. Drop the old table if it exists (be careful with this in production!)
DROP TABLE IF EXISTS event_registrations CASCADE;
DROP TABLE IF EXISTS events CASCADE;

-- 2. Create events table with correct column name
CREATE TABLE events (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  title VARCHAR(255) NOT NULL,
  slug VARCHAR(255) UNIQUE NOT NULL,
  description TEXT,
  event_date DATE NOT NULL,  -- Changed from 'date' to 'event_date'
  start_time TIME,
  end_time TIME,
...
-- 5. Create indexes for registrations
...
CREATE POLICY "Allow public read access to registrations"
  ON event_registrations FOR SELECT TO public USING (true);

CREATE POLICY "Allow authenticated users to manage registrations"
  ON event_registrations FOR ALL TO authenticated USING (true);

-- 8. Insert sample events
INSERT INTO events (title, slug, description, event_date, start_time, end_time, location, category, image_url, price, capacity, status, is_featured)
VALUES 
(
